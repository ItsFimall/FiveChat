...
const App: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    async function initData() {
      localStorage.setItem('five_chat_app_status', 'installed');
      localStorage.setItem('five_chat_app_version', '1.0.0');
    }
    const status = localStorage.getItem('five_chat_app_status');
    if (status !== 'installed') {
      initData().then(() => {
